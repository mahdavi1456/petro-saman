<?php include"../header.php"; include"../nav.php"; include('function.php');
	$aru = new aru();
	$asb = $aru->list_by_type("secretariat", "s_category", "meeting", "string", "s_id");
	$type = $aru->list_by_type("doc_type", "s_category", "meeting", "string", "d_id");
			...
			<div class="box-header with-border">
				<div class="box-body">
					<form action="" method="post">
						<input type="hidden" name="s_category" value="meeting">
						<div class="row">
				...
				<?php 
					if($asb){
						...
						foreach ($asb as $a ) {
							$s_id = $a['s_id'];
							$s_type = $a['s_type'];
							$doc_type = $aru->field_by_type("doc_type","s_type","d_id",$s_type,"int");
						?>
						<tr>
							<td><?php echo ($row); ?></td>
							<td><?php echo per_number($doc_type); ?></td>
																...
																<?php
																	if($type)
																	{
																		...
																		foreach ($type as $b ) 
																		{
																			$d_id = $b['d_id'];
																		?>
																		<option <?php if($d_id==$s_type) echo "selected"; ?> value="<?php echo per_number($d_id); ?>"><?php echo $b['s_type']; ?></option>
																		<?php
